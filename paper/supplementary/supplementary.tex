...
\large Structure-predicted solvent responses enable simulation-free hydration free-energy prediction}
...
\subsection*{S10. Lambda-response probes}
...
\subsection*{S11. Artifact verification}

The public API accepts one or more SMILES strings. It canonicalizes each structure,
computes descriptors, evaluates the two D-MPNNs and four tree-artifact families, and
...
identified in the model card and manifest.

\subsection*{S12. Runtime benchmarking}
...
\subsection*{S3. Simulation-assisted analyses}

An earlier branch evaluated routers and selective PIMD fallback. Those results are
retained as an explicitly non-deployable reference in Extended Data Fig.~4 and the
experiment ledger. They do not define SolvAI, do not enter its predictions and are
not used to support the simulation-free inference claim.
